...
exports.contactcontroller = catchAsyncErrors(async (req, res, next) => {
    try {
        const { name, email,date, country, countrycode, visacountry, whatsupcountrycode, whatsupnumber, message, number } = req.body;
        const data = await Contact.create({
            name, email, date,country, countrycode, visacountry, whatsupcountrycode, whatsupnumber, message, number
        })
        res.status(201).json({
            success: true,
            data
